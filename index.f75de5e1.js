...
container.addEventListener("click", (e)=>{
    ...
    if (target.classList.contains("remove-column")) {
        ...
        removeColumn();
        if (field.rows[0].cells.length <= minCellsRows) target.disabled = true;
    }
});
function appendRow() {
    const newRow = field.insertRow(field.rows.length - 1);
    const cellsToAdd = field.rows[0].cells.length;
    for(let i = 0; i < cellsToAdd; i++)newRow.insertCell(i);
}
function removeRow() {
...
function appendColumn() {
    ...
    for(let i = 0; i < rowsToAdd; i++)field.rows[i].insertCell();
}
function removeColumn() {
    const cellsToDelete = field.rows.length;
    const lastCell = field.rows[0].length - 1;
    for(let i = 0; i < cellsToDelete; i++)field.rows[i].deleteCell(lastCell);
}
